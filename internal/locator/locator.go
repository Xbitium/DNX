...
// Table size = number of distinct children at this router = O(children),
// NOT O(hosts). That is the whole scaling claim.
type Table struct {
	tierDepth int
...
// It reads EXACTLY ONE tier and decides. Never the whole name, never an IP.
